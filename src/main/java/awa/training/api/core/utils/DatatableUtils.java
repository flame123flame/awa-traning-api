...
import awa.training.api.common.model.DatatableFilter;
import awa.training.api.common.model.DatatableSort;
...
public class DatatableUtils {
	...
	public static String countForDatatableGroupBy(String sql,String groupBy, List<DatatableFilter> filter) {
		StringBuilder sqlBuilger = new StringBuilder();
		sqlBuilger.append("select count(1) from  ");
		if (sql.startsWith("select") || sql.startsWith("SELECT")) {
			sqlBuilger.append(" (");
	...
	private static String createSortFilterString(List<DatatableSort> sort, List<DatatableFilter> filter) {
		...
		if (filter.size() != 0 && filter != null) {
			...
			for (DatatableFilter item : filter) {
				...
				} else if (item.getOp().trim().toLowerCase().equals("in")) {
					sqlBuilger.append(" in ");
					sqlBuilger.append(item.getValue());

				}else if (item.getOp().trim().toLowerCase().equals("custom")) {
				} else {
	...
	///////////////////////////// with out tb /////////////////////////////////////
	public static String limitForDataTableCustom(String sql, int page, int length, List<DatatableSort> sort,
			List<DatatableFilter> filter) {
		page = page * length;
		StringBuilder sqlBuilger = new StringBuilder();
		sqlBuilger.append(sql);
